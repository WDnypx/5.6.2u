        ...
        if (!allRemoteSort && orderColumns.length) {
            if (isRowGroupStatus) {
                ...
                if (allSortMethod) {
                    ...
                    tableTree = XEUtils.toArrayTree(XEUtils.orderBy(treeList, orderColumns.map(({ column, order }) => [getOrderField($xeTable, column), order])), {
                        key: aggregateOpts.rowField,
                        parentKey: aggregateOpts.parentField,
...
const Methods = {
    ...
    triggerRadioRowEvent(evnt, params) {
        ...
        if (!checkMethod || checkMethod({ $table: $xeTable, row })) {
            ...
            if (isChange) {
                $xeTable.dispatchEvent('radio-change', Object.assign({ oldValue, newValue }, params), evnt);
            }
        }
    },
    triggerCurrentColumnEvent(evnt, params) {
        ...
        if (!beforeRowMethod || beforeRowMethod({ column: newValue, $table: $xeTable })) {
            ...
            if (isChange) {
                $xeTable.dispatchEvent('current-column-change', Object.assign({ oldValue, newValue }, params), evnt);
            }
        }
        else {
    ...
    triggerCurrentRowEvent(evnt, params) {
        ...
        if (!beforeRowMethod || beforeRowMethod({ row: newValue, $table: $xeTable })) {
            ...
            if (isChange) {
                $xeTable.dispatchEvent('current-row-change', Object.assign({ oldValue, newValue }, params), evnt);
                // 已废弃
                $xeTable.dispatchEvent('current-change', Object.assign({ oldValue, newValue }, params), evnt);
            }
        }
        else {
