...
const methods = {};
const propKeys = Object.keys(_props.default);
...
function getTableOns(_vm) {
  ...
  const ons = {};
  _xeUtils.default.each($listeners, (cb, type) => {
    ons[type] = (...args) => {
      _vm.$emit(type, ...args);
    };
  });
...
function renderForm(h, _vm) {
  ...
  const hasForm = !!(formSlot || (0, _utils.isEnableConf)(formConfig));
  if (hasForm) {
    return h('div', {
      key: 'form',
      ref: 'refFormWrapper',
...
function renderToolbar(h, _vm) {
  ...
  const hasToolbar = !!(toolbarSlot || (0, _utils.isEnableConf)(toolbarConfig) || toolbar);
  if (hasToolbar) {
    return h('div', {
      key: 'toolbar',
...
function renderLayout(h, $xeGrid) {
  ...
  }, asideLeftSlot({})) : renderEmptyElement($xeGrid), h('div', {
...
Object.keys(_table.default.methods).forEach(name => {
...
var _default = exports.default = {
  ...
  computed: {
    ...
    computeProxyOpts() {
      return _xeUtils.default.merge({}, _xeUtils.default.clone(getConfig().grid.proxyConfig, true), this.proxyConfig);
    },
    pagerOpts() {
  ...
  created() {
    ...
    if (this.toolbarConfig && !_xeUtils.default.isObject(this.toolbarConfig)) {
      (0, _log.warnLog)('vxe.error.errProp', [`grid.toolbar-config=${this.toolbarConfig}`, 'grid.toolbar-config={}']);
    }
    // if (proxyOpts.props) {
  ...
  methods: Object.assign(Object.assign({}, methods), {
    ...
    getParentHeight() {
      ...
      if (el) {
        const parentEl = el.parentElement;
        return (reactData.isZMax ? (0, _dom.getDomNode)().visibleHeight : parentEl ? _xeUtils.default.toNumber(getComputedStyle(parentEl).height) : 0) - $xeGrid.getExcludeHeight();
      }
      return 0;
    },
    initToolbar() {
      ...
      if (proxyConfig) {
        ...
        if (!proxyInited && proxyOpts.autoLoad !== false) {
          ...
          this.$nextTick().then(() => this.commitProxy('_init')).then(rest => {
            this.$emit('proxy-query', Object.assign(Object.assign({}, rest), {
              isInited: true,
              $grid: this,
    ...
    commitProxy(proxyTarget, ...args) {
      ...
      if (_xeUtils.default.isString(proxyTarget)) {
        const matchObj = toolbarConfig || toolbar ? _xeUtils.default.findTree(toolbarOpts.buttons, item => item.code === proxyTarget, {
          children: 'dropdowns'
        }) : null;
        code = proxyTarget;
    ...
    getFormItems(itemIndex) {
      ...
      const itemList = [];
      _xeUtils.default.eachTree((0, _utils.isEnableConf)(formConfig) && formOpts.items ? formOpts.items : [], item => {
        itemList.push(item);
      }, {
        children: 'children'
      });
    ...
    triggerToolbarCommitEvent(params, evnt) {
      ...
      return this.commitProxy(params, evnt).then(rest => {
        if (code && rest && rest.status && ['query', 'reload', 'delete', 'save'].includes(code)) {
          this.$emit(code === 'delete' || code === 'save' ? `proxy-${code}` : 'proxy-query', Object.assign(Object.assign({}, rest), {
    ...
    sortChangeEvent(params) {
      ...
      const isRemote = _xeUtils.default.isBoolean(column.remoteSort) ? column.remoteSort : $table.sortOpts.remote;
      // 如果是服务端排序
      if (isRemote) {
        this.sortData = sortList;
        if (this.proxyConfig) {
    ...
    handleZoom(isMax) {
      ...
      if (isMax ? !isZMax : isZMax) {
        this.isZMax = !isZMax;
        if (this.tZindex < (0, _utils.getLastZIndex)()) {
